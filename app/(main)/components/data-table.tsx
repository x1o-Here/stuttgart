...
export function DataTable<TData, TValue>({
  ...
  const maxSellingPrice = useMemo(() => {
    ...
    return Math.max(
      ...vehicles.map((v) =>
        Math.max(v.salesDetails?.salesAmount || 0, v.totalCost || 0),
      ),
    );
  }, [vehicles]);
  ...
  const [sorting, setSorting] = useState([
    { id: tab === "active" ? "purchasedDate" : "soldDate", desc: true },
  ]);
      ...
      <div className="rounded-lg w-full flex flex-col gap-2 text-gray-400">
        ...
        <div className="w-full flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <SortPopover
              options={
                tab === "active" ? ACTIVE_SORT_OPTIONS : SOLD_SORT_OPTIONS
              }
            ...
            <div className="flex items-center gap-2">
              <PurchasedDateFilterPopover
                ...
                onDateChange={(from, to) =>
                  setColumnFilters([
                    {
                      id: tab === "active" ? "purchasedDate" : "soldDate",
                      value: { from, to },
                    },
                  ])
                }
