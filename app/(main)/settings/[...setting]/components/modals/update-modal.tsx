...
import { formSchema, FormSchema } from "./create-modal";
...
export default function UpdateModal({ entityLabel, initialData }: UpdateModalProps) {
    ...
    async function onSubmit(data: FormSchema) {
        ...
        try {
            await store.updateItem(activeCompany, user.uid, collectionKey, initialData.id, {
                name: data.name,
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" size="icon" className="cursor-pointer w-8 h-8">
                    <Pencil />
                </Button>
            </DialogTrigger>
                        ...
                        <FieldGroup className="grid grid-cols-5 gap-6">
                            <Controller
                                ...
                                render={({ field, fieldState }) => (
                                    <Field data-invalid={fieldState.invalid} className={isVehicle ? "col-span-5" : "col-span-4"}>
                                        ...
                                        <Input
                                            ...
                                            placeholder={isVehicle ? "e.g. MH-12-XX-1234" : `Enter ${entityLabel.toLowerCase()} name`}
                                            autoComplete="off"
                                        />
                                        {fieldState.invalid && (
