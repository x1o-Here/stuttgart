...
export function VehicleProvider({
  ...
  useEffect(() => {
      ...
      if (data?.purchasedDate) {
        setVehicle((prev: any) =>
          prev
            ? {
                ...prev,
                months: calculateMonthsSincePurchase(data.purchasedDate),
              }
            : prev,
        );
      }
    ...
    // Purchase payments
    const purchasePaymentsCol = collection(
      db,
      "purchaseDetails",
      vehicleId,
      "purchasePayments",
    );
    const unsubscribePurchasePayments = onSnapshot(
      purchasePaymentsCol,
      (snap) => {
    ...
    const unsubscribeQuotations = onSnapshot(quotationsCol, (snap) => {
      ...
      vehicleQuotations.forEach((quotation) => {
        ...
        const unsub = onSnapshot(paymentsCol, (paySnap) => {
          ...
          setQuotations((prev) =>
            prev.map((q) => (q.id === quotation.id ? { ...q, payments } : q)),
          );
        });
  ...
  useEffect(() => {
    ...
    const totalQuotationAmount =
      quotations?.reduce((acc, q) => acc + (q.data.amount || 0), 0) || 0;
